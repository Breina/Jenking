...
func NewProjectBuildsProvider(client jmodel.JenkinsClient, store *cache.Store, nc NavigationContext) *ProjectBuildsProvider {
...
func (p *ProjectBuildsProvider) HandleMsg(msg tea.Msg) (bool, []tea.Cmd) {
	...
	case projectBuildsResultMsg:
		...
		sort.Slice(p.builds, func(i, j int) bool {
			return p.builds[i].Timestamp.After(p.builds[j].Timestamp)
		})
		if p.store != nil && p.store.Registry != nil {
