...
func fromPalette(id ThemeID, name string, p palette) Theme {
	...
	return Theme{
		...
		Log: LogStyles{
			...
			CurrentHighlight: lipgloss.NewStyle().Foreground(dark).Background(p.failed).Bold(true),
		},
		Stage: StageStyles{
...
func Matrix() Theme {
	...
	t := Theme{
		...
		Log: LogStyles{
			...
			CurrentHighlight: lipgloss.NewStyle().Foreground(black).Background(neon).Bold(true),
		},
		Stage: StageStyles{
