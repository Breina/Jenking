...
type DescribeView struct {
	...
	issues []jenkins.ValidationIssue
...
func NewDescribeView(t theme.Theme, client jenkins.JenkinsClient, store *cache.Store, nc NavigationContext, build jenkins.Build) *DescribeView {
	...
	dv.scriptLV = LogViewer{
		theme:            t,
		highlightErrors:  true,
		currentMatchLine: -1,
		currentNavIdx:    -1,
		classifyFn: func(rawIdx int, _ string) lineKind {
...
func (dv *DescribeView) Init() tea.Cmd {
	dv.buildParamLines()
	dv.buildScriptLines()
	if vimPolicy.Enabled && vimPolicy.PrefetchSymbols {
	...
	return func() tea.Msg {
		...
		if store != nil && store.Symbols != nil {
			if e := store.Symbols.Get(key); e != nil && e.Value != nil {
				applyOverlays(e.Value)
				return describeSymbolsMsg{symbols: e.Value}
			}
			if store.Disk != nil {
				if sym, err := store.Disk.LoadSymbols(key); err == nil {
		...
		hasContent := sym != nil && (len(sym.Steps) > 0 || len(sym.Globals) > 0)
		if hasContent && store != nil && store.Symbols != nil {
			store.Symbols.Put(key, sym)
			if store.Disk != nil {
...
func (dv *DescribeView) applyValidationResult(res jenkins.ValidationResult) {
	if res.OK {
		dv.issues = nil
	} else {
...
func (dv *DescribeView) renderFooter() string {
	...
	if dv.scriptW > 0 {
		msg, _ = truncateToColumns(msg, dv.scriptW)
	}
...
func (dv *DescribeView) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case editorDoneMsg:
		...
		if msg.err == nil && msg.tmpFile != "" {
			if data, err := os.ReadFile(msg.tmpFile); err == nil {
				...
				dv.buildScriptLines()
				if vimPolicy.Enabled && vimPolicy.ValidateOnSave {
	...
	case editorValidatedMsg:
		...
		if msg.err != nil {
			err := msg.err
			return dv, func() tea.Msg { return ErrorMsg{Err: fmt.Errorf("validate: %w", err)} }
		}
...
func (dv *DescribeView) openEditorCmd() tea.Cmd {
	...
	if rt != nil && vimPolicy.ValidateOnSave {
...
func (dv *DescribeView) Shortcuts() []component.Shortcut {
	...
	if dv.scriptLV.searchRe != nil || dv.scriptLV.ErrorNavActive() {
