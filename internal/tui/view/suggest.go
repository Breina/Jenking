...
func branchCandidates(store *cache.Store, projectPath, prefix string) []string {
	...
	var out []string
	// Primary source: the Jobs cache populated when the user opens the
	// multibranch project's job list — each child Job is a branch.
	if store.Jobs != nil {
		if e := store.Jobs.Get(projectPath); e != nil {
