...
// jenkins.JenkinsClient. Defined as its own interface so tests can pass a
// one-method stub.
type jenkinsfileValidator interface {
	ValidateJenkinsfile(ctx context.Context, content string) (jenkins.ValidationResult, error)
}
...
func runVimValidator(ctx context.Context, client jenkinsfileValidator, rt *vimRuntime, done <-chan struct{}, pollInterval time.Duration) {
	...
	for {
		...
		if err != nil {
			...
			writeErrorsQF(rt.errorsQF, jenkins.ValidationResult{
				Issues: []jenkins.ValidationIssue{{Message: "validator: " + err.Error()}},
			}, rt.bufferTmp)
			continue
		}
...
func writeErrorsQF(path string, res jenkins.ValidationResult, file string) {
