...
func (a App) View() string {
	...
	if hasPreview {
		...
		previewBC.SetSegment(&component.BreadcrumbSegment{
			...
			Context:  viewToComponentParts(seg.Context),
		})
...
func viewToComponentParts(parts []view.BreadcrumbPart) []component.BreadcrumbPart {
	out := make([]component.BreadcrumbPart, len(parts))
	for i, p := range parts {
		out[i] = component.BreadcrumbPart{
			Text:       p.Text,
			IsBuildNum: p.IsBuildNum,
