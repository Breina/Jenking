...
type BreadcrumbSegment struct {
	...
	Context  []BreadcrumbPart // parts joined by "/" (or ":" for the last separator)
}

// BreadcrumbPart is one piece of the context portion of a breadcrumb segment.
type BreadcrumbPart struct {
	Text       string
	IsBuildNum bool   // render in green with # prefix
	Separator  string // override separator before this part (":" instead of "/"); empty = default "/"
}
...
func jobRefParts(projectName, branchName string) []BreadcrumbPart {
	parts := []BreadcrumbPart{{Text: shortName(decodeName(projectName))}}
	if branchName != "" {
		parts = append(parts, BreadcrumbPart{Text: decodeName(branchName)})
	}
	return parts
}
