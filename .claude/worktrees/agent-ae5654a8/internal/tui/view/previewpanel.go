...
func (pp *PreviewPanel) Breadcrumb(stages []jenkins.Stage, pending bool) BreadcrumbSegment {
	...
	if !pending && pp.buildNumber > 0 {
		ctx = append(ctx, BreadcrumbPart{Text: fmt.Sprintf("%d", pp.buildNumber), IsBuildNum: true})
	}
	if pp.pipelineConsole {
		ctx = append(ctx, BreadcrumbPart{Text: "Pipeline", Separator: ":"})
	} else if pp.stageIdx >= 0 && pp.stageIdx < len(stages) {
		ctx = append(ctx, BreadcrumbPart{Text: stages[pp.stageIdx].Name, Separator: ":"})
	}
